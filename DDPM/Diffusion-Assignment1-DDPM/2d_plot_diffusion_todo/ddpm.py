...
class DiffusionModule(nn.Module):
    ...
    def q_sample(self, x0, t, noise=None):
        ...
        xt = np.sqrt(alphas_prod_t) * x0 + np.sqrt(1-alphas_prod_t)*noise
        #######################
    ...
    def p_sample(self, xt, t):
        ...
        beta_t_sqrt = (1-extract(self.var_scheduler.alphas, t, xt)).sqrt()
        alpha_t_sqrt = (extract(self.var_scheduler.alphas, t, xt)).sqrt()
        noise = torch.randn_like(xt) if t.item() > 0 else 0.0

        x_t_prev = (1/alpha_t_sqrt) * (xt-(eps_factor)*eps_theta) + beta_t_sqrt * noise
